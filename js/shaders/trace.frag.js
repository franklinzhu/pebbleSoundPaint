...
void main(){
    ...
    vec2 off =  u_audio_high * 2.0 * curlNoise(vec3(v)).xy * u_audio_mid;
    vec2 m_uv2 = v_uv;

    vec4 img = texture2D(r_tex_brush, m_uv2) + texture2D(r_tex_trace, (0.5+0.5*(1.0*(-1.0+2.0*m_uv2)))+off*u_wiggle);;
    gl_FragColor = pow(clamp(img-disp, 0.0, 1.0), vec4(1.002*(1.0+disp)));
}
`;
